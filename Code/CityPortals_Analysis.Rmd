...
merged %>%
  ...
  write.csv("~/Documents/Github/JCDL-Extended/Data/Cities_with_multiple_library_systems")
```
...
merged_PropLR <- lm(ProportionPublicLibData ~ OpenDataCensusScores, data=merged)
summary(merged_PropLR)
```

```{r}
# Simple linear regression model
merged_VettLR <- lm(CountVettedPublicLibData ~ OpenDataCensusScores, data=merged)
summary(merged_VettLR)
```

```{r}
# Simple linear regression model
merged_TotLR <- lm(TotalDatasetsAvailable ~ OpenDataCensusScores, data=merged)
summary(merged_TotLR)
```

```{r}
# Simple linear regression model
merged_PropSizeLR <- lm(ProportionPublicLibData ~ LOCALE_ADD_DESCR, data=merged)
summary(merged_PropSizeLR)
```

```{r}
# Simple linear regression model
merged_VettSizeLR <- lm(CountVettedPublicLibData ~ LOCALE_ADD_DESCR, data=merged)
summary(merged_VettSizeLR)
```

```{r}
# Simple linear regression model
merged_TotSizeLR <- lm(TotalDataSetsAvailable ~ LOCALE_ADD_DESCR, data=merged)
summary(merged_TotSizeLR)
```
