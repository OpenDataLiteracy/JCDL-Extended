...
portals %>% tidyr::pivot_longer(
  ...
  ggplot(aes(Category, GrandTotal)) +
    ...
    scale_x_discrete(labels=c("LibraryDataCategories_CatalogAndCirculation" = "Catalog & Circulation",
                              "LibraryDataCategories_EventsAndOutreach" = "Events & dOutreach",
...
portals %>%
  count(Software) %>%
  ggplot(aes(factor(Software, level = c("Arcgis", "CKAN", "DKAN", "JKAN", "Junar", "Opendatasoft", "Socrata", "Other")), n)) +
    geom_bar(stat="identity", fill="steelblue") +
    theme(axis.text.x = element_text(angle = 60, hjust = 1, size = 9)) +
    geom_text(aes(label=n), vjust=1.1, color="white", size=3) +
    scale_x_discrete(labels=c("Arcgis", "CKAN", "DKAN", "JKAN", "Junar", "Opendatasoft", "Socrata", "Other", "No Portal Found")) +
    xlab("Portal Software") +
    ylab("Total Count of Portals") 

#ggsave("~/Documents/Github/JCDL-Extended/Images/software_barchart.png")
...
```{r}
#https://twitter.com/KellyBodwin/status/1272730871561244675?s=20
# linear regression the dplyr way
single_libs %>%
    with(lm(ProportionPublicLibData ~ OpenDataCensusScore2017)) %>%
    summary()
```
...
single_libs %>%
    with(lm(CountVettedPublicLibData ~ OpenDataCensusScore2017)) %>%
    summary()
```
...
# Simple linear regression model
single_libs %>%
    with(lm(log(CountVettedPublicLibData + 0.0000001) ~ OpenDataCensusScore2017)) %>%
    summary()
```
...
# Simple linear regression model
single_libs %>%
    with(lm(TotalDataSetsAvailable ~ OpenDataCensusScore2017)) %>%
    summary()
```
